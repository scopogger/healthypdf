...
class ImagePlacementDialog(QDialog):
    ...
    def _refresh_preview(self):
        ...
        # Image rectangle — show actual image scaled to fit the placement rect
        img_abs_x = page_x0 + img_x
        img_abs_y = page_y0 + img_y
        img_rect_f = QRectF(img_abs_x, img_abs_y, img_px_w, img_px_h)

        if img_px_w > 1 and img_px_h > 1:
            img_pm = QPixmap(self.image_path)
            if not img_pm.isNull():
                scaled = img_pm.scaled(
                    int(img_px_w), int(img_px_h),
                    Qt.KeepAspectRatio,
                    Qt.SmoothTransformation
                )
                # Re-center after KeepAspectRatio scaling (may be smaller than slot)
                draw_x = img_abs_x + (img_px_w - scaled.width()) / 2
                draw_y = img_abs_y + (img_px_h - scaled.height()) / 2
                p.setClipRect(img_rect_f)
                p.drawPixmap(int(draw_x), int(draw_y), scaled)
                p.setClipping(False)
            else:
                # Fallback if image can't be loaded as QPixmap
